...
double TSRIM::GetAdjustedEnergy(double energy,double thickness,double stepsize)	{
	...
	double xstep = stepsize, xtot = 0.0; // MAKE XSTEP SMALLER FOR BETTER RESULTS. 1UM SHOULD BE FINE ... UNLESS YOU ARE AT THE BRAGG PEAK ??
...
//				printf("energy_temp = %.02f\t DEDX = %.02f\n",energy_temp,fEnergyLoss->Eval(energy_temp,0,"S"));
				energy_temp -= xstep * sEnergyLoss->Eval(energy_temp); // update energy recursively so that it decreases each step
				xtot += xstep;
				if(energy_temp<=0.0)
