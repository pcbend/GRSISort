...
//  -> Reconstructing excitation energies lead to angle-independent errors which scale with exc 
//  -> exc_rec = r->GetExcEnergy(r->GetTLab(theta*d2r,2),theta*d2r,2) != fExc
//
// *********** *********** *********** *********** *********** *********** *********** //
...
public:
  TReaction(const char *beam, const char *targ, const char *ejec, const char *reco, double ebeam=0.0, double ex3=0.0, bool inverse=false);
	...
	void InitReaction();
  void SetExc(double exc) { SetCmFrame(exc); } 
  ...
  const char *GetIonName(int part) {return fIonName[part].c_str();}
